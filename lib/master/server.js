...
var Server = function (conf) {
  ...
  setInterval(function () {
    rserver.log.info(
      "Nodes: " +
        __(rserver.nodes).size() +
        ", " +
        "WebClients: " +
        __(rserver.web_clients).size()
    );
  }, STATUS_INTERVAL);
};
...
Server.prototype = {
  ...
  announce_web_client: function (socket) {
    ...
    setInterval(function () {
      rserver.io.sockets
        .in("web_clients")
        .emit("statusreport", { status: rserver.status });
    }, STATUS_INTERVAL / 10);
    socket.on("webreport", function (message) {
      if (rserver.status == STATUS_RUNNING) {
        socket.emit(
          "webreport",
          rserver.runconfig.agent,
          rserver.runconfig.maxuser,
          stat.getTimeData(rserver),
          stat.getCountData()
        );
      }
    });
    ...
    rserver.io.sockets.on("connection", function (socket) {
      ...
      socket.on("announce_web_client", function (message) {
        ...
        socket.on("run", function (msg) {
          ...
          console.log("server begin notify client to run machine...", msg);
